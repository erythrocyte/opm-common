...
namespace
{
    namespace str
    {
        ...
        */
        ...
        inline std::string_view del_after_first_slash(std::string_view view)
        {
            ...
            const auto term = [](itr begin, itr end) { return std::find(begin, end, '/'); };
        ...
        {
            auto count = std::count_if(code_keywords.begin(),
                                       code_keywords.end(),
    ...
    public:
        ...
        const std::set<Opm::Ecl::SectionType>& get_ignore()
        {
    ...
    bool ParserState::done() const
    {
        ...
        while (!this->input_stack.empty() && this->input_stack.top().input.empty())
    ...
    void ParserState::loadFile(const std::filesystem::path& inputFile)
    {
        ...
        std::unique_ptr<std::FILE, decltype(closer)> ufp(std::fopen(inputFile.c_str(), "rb"), closer);
        ...
        if (std::ferror(fp) || readc != buffer.size() - 1)
            throw std::runtime_error("Error when reading input file '" + inputFile.string() + "'");
    ...
    std::optional<std::filesystem::path> ParserState::getIncludeFilePath(std::string path) const
    {
        ...
        static const std::string validPathNameCharacters(
        ...
        try {
            ...
            parseContext.handleError(ParseContext::PARSE_MISSING_INCLUDE,
                                     fmt::format("No such file: {}", includeFilePath.string()),
                                     {},
                                     errors);
            return {};
        }
    ...
    RawKeyword* newRawKeyword(const ParserKeyword& parserKeyword,
                              ...
                              const Parser& parser)
    {
        for (const auto& keyword : parserKeyword.prohibitedKeywords()) {
            if (parserState.deck.hasKeyword(keyword)) {
                parserState.parseContext.handleError(
                    ParseContext::PARSE_INVALID_KEYWORD_COMBINATION,
                    fmt::format("Incompatible keyword combination: {} declared when {} is already present.",
                                keywordString,
                                keyword),
        ...
        for (const auto& keyword : parserKeyword.requiredKeywords()) {
            if (!parserState.deck.hasKeyword(keyword)) {
                parserState.parseContext.handleError(
                    ParseContext::PARSE_INVALID_KEYWORD_COMBINATION,
                    fmt::format(
                        "Incompatible keyword combination: {} declared, but {} is missing.", keywordString, keyword),
                    KeywordLocation {keywordString, parserState.current_path(), parserState.line()},
                    parserState.errors);
            }
        }
        ...
        if (parserKeyword.getSizeType() == SLASH_TERMINATED || parserKeyword.getSizeType() == UNKNOWN
            ...
            case DOUBLE_SLASH_TERMINATED:
                rawSizeType = Raw::DOUBLE_SLASH_TERMINATED;
                break;
            default:
        ...
        std::string msg_fmt = fmt::format(
            "Problem with {{keyword}} - missing {0}\n"
            "In {{file}} line {{line}}\n"
            "For the keyword {{keyword}} we expect to read the number of records from keyword {0}, {0} was not found",
            keyword_size.keyword());
        parserState.parseContext.handleError(
            ParseContext::PARSE_MISSING_DIMS_KEYWORD,
            msg_fmt,
    ...
    RawKeyword* newRawKeyword(const std::string& deck_name,
                              ParserState& parserState,
                              const Parser& parser,
                              const std::string_view& line)
    {
        if (deck_name.size() > RawConsts::maxKeywordLength) {
            ...
            if (parser.isRecognizedKeyword(keyword8)) {
                std::string msg = "Keyword {keyword} to long - only eight first characters recognized\n"
                                  "In {file} line {line}\n";
                parserState.parseContext.handleError(
                    ParseContext::PARSE_LONG_KEYWORD,
                    msg,
    ...
    std::unique_ptr<RawKeyword> tryParseKeyword(ParserState& parserState, const Parser& parser)
    {
        ...
        while (!parserState.done()) {
            ...
            if (line.empty() && !rawKeyword)
                continue;
            if (line.empty() && !is_title)
                continue;
                ...
                */
                ...
                if (is_title) {
                    if (record_buffer.empty()) {
                        ...
                        auto mm = std::string_view {record_buffer.begin(), size};
                        RawRecord record(mm, rawKeyword->location());
                        rawKeyword->addRecord(record);
                    }
                    return rawKeyword;
                }
        ...
        if (ignore.size() > 0)
            if (!parserState.check_section_keywords())
                throw std::runtime_error(
                    "Parsing individual sections not possible when section keywords in root input file");
            ...
            if (parser.isRecognizedKeyword(rawKeyword->getKeywordName())) {
                ...
                const auto& parserKeyword = parser.getParserKeywordFromDeckName(kwname);
                {
                    ...
                    auto msg = fmt::format("{:5} Reading {:<8} in {} line {}",
                                           parserState.deck.size(),
                                           rawKeyword->getKeywordName(),
                                           location.filename,
                                           location.lineno);
                    OpmLog::info(msg);
                }
                try {
                    if (rawKeyword->getKeywordName() == Opm::RawConsts::pyinput) {
                        ...
                        if (deck_keyword.name() == ParserKeywords::IMPORT::keywordName) {
                            ...
                            const auto& import_file = parserState.getIncludeFilePath(
                                deck_keyword.getRecord(0).getItem(0).get<std::string>(0));

                            ImportContainer import(parser,
                                                   parserState.deck.getActiveUnitSystem(),
                                                   import_file.value().string(),
                                                   formatted,
                                                   parserState.deck.size());
                            for (auto kw : import)
                ...
                KeywordLocation location(
                    rawKeyword->getKeywordName(), parserState.current_path().string(), parserState.line());
 ...
 */
std::string
Parser::stripComments(const std::string& str)
{
    return {str.begin(), str::find_terminator(str.begin(), str.end(), str::find_comment())};
}
...
inline void
assertFullDeck(const ParseContext& context)
{
...
EclipseState
Parser::parse(const std::string& filename, const ParseContext& context, ErrorGuard& errors)
{
...
EclipseState
Parser::parse(const Deck& deck, const ParseContext& context)
{
    assertFullDeck(context);
...
EclipseState
Parser::parseData(const std::string& data, const ParseContext& context, ErrorGuard& errors)
{
    assertFullDeck(context);
    Parser p;
...
EclipseGrid
Parser::parseGrid(const std::string& filename, const ParseContext& context, ErrorGuard& errors)
{
    if (context.hasKey(ParseContext::PARSE_MISSING_SECTIONS))
...
EclipseGrid
Parser::parseGrid(const Deck& deck, const ParseContext& context)
{
    if (context.hasKey(ParseContext::PARSE_MISSING_SECTIONS))
...
EclipseGrid
Parser::parseGridData(const std::string& data, const ParseContext& context, ErrorGuard& errors)
{
    Parser parser;
...
Deck
Parser::parseFile(const std::string& dataFileName,
                  const ParseContext& parseContext,
                  ErrorGuard& errors,
                  const std::vector<Opm::Ecl::SectionType>& sections) const
{
    ...
    if (sections.size() > 0) {
        ...
        std::set_difference(all_sections.begin(),
                            all_sections.end(),
                            read_sections.begin(),
                            read_sections.end(),
...
Deck
Parser::parseFile(const std::string& dataFileName, const ParseContext& parseContext) const
{
    ErrorGuard errors;
...
Deck
Parser::parseFile(const std::string& dataFileName) const
{
    ErrorGuard errors;
    return this->parseFile(dataFileName, ParseContext(), errors);
}
...
Deck
Parser::parseString(const std::string& data, const ParseContext& parseContext, ErrorGuard& errors) const
{
...
Deck
Parser::parseString(const std::string& data, const ParseContext& parseContext) const
{
    ErrorGuard errors;
...
Deck
Parser::parseString(const std::string& data) const
{
    ErrorGuard errors;
    return this->parseString(data, ParseContext(), errors);
}
...
const ParserKeyword*
Parser::matchingKeyword(const std::string_view& name) const
{
...
bool
Parser::hasWildCardKeyword(const std::string& internalKeywordName) const
{
    return (m_wildCardKeywords.count(internalKeywordName) > 0);
}

bool
Parser::isRecognizedKeyword(const std::string_view& name) const
{
...
void
Parser::addParserKeyword(ParserKeyword parserKeyword)
{
...
void
Parser::addParserKeyword(const Json::JsonObject& jsonKeyword)
{
    addParserKeyword(ParserKeyword(jsonKeyword));
}

bool
Parser::hasKeyword(const std::string& name) const
{
    return this->m_deckParserKeywords.find(std::string_view(name)) != this->m_deckParserKeywords.end();
}

const ParserKeyword&
Parser::getKeyword(const std::string& name) const
{
    return getParserKeywordFromDeckName(std::string_view(name));
}

const ParserKeyword&
Parser::getParserKeywordFromDeckName(const std::string_view& name) const
{
    auto candidate = m_deckParserKeywords.find(name);

    if (candidate != m_deckParserKeywords.end())
...
void
Parser::loadKeywords(const Json::JsonObject& jsonKeywords)
{
...
bool
Parser::loadKeywordFromFile(const std::filesystem::path& configFile)
{
...
void
Parser::loadKeywordsFromDirectory(const std::filesystem::path& directory, bool recursive)
{
...
static bool
isSectionDelimiter(const DeckKeyword& keyword)
{
    const auto& name = keyword.name();
    for (const auto& x : {"RUNSPEC", "GRID", "EDIT", "PROPS", "REGIONS", "SOLUTION", "SUMMARY", "SCHEDULE"})
        if (name == x)
            return true;
...
bool
DeckSection::checkSectionTopology(const Deck& deck, const Parser& parser, bool ensureKeywordSectionAffiliation)
{
    if (deck.size() == 0) {
    ...
    for (; curKwIdx < deck.size(); ++curKwIdx) {
        ...
        if (!isSectionDelimiter(curKeyword)) {
            ...
            if (ensureKeywordSectionAffiliation && !parserKeyword.isValidSection(curSectionName)) {
                std::string msg = "The keyword '" + curKeywordName + "' is located in the '" + curSectionName
        ...
        if (curSectionName == "RUNSPEC") {
            if (curKeywordName != "GRID") {
                std::string msg = "The RUNSPEC section must be followed by GRID instead of " + curKeywordName;
                OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
                deckValid = false;
            }
            ...
            if (curKeywordName != "EDIT" && curKeywordName != "PROPS") {
                std::string msg = "The GRID section must be followed by EDIT or PROPS instead of " + curKeywordName;
                OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
                deckValid = false;
            }
            ...
            if (curKeywordName != "PROPS") {
                std::string msg = "The EDIT section must be followed by PROPS instead of " + curKeywordName;
                OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
                deckValid = false;
            }
            ...
            if (curKeywordName != "SOLUTION") {
                std::string msg = "The REGIONS section must be followed by SOLUTION instead of " + curKeywordName;
                OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
                deckValid = false;
            }
            ...
            if (curKeywordName != "SCHEDULE") {
                std::string msg = "The SUMMARY section must be followed by SCHEDULE instead of " + curKeywordName;
                OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
                deckValid = false;
            }
            ...
            std::string msg
                = "The SCHEDULE section must be the last one (" + curKeywordName + " specified after SCHEDULE)";
            OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
            deckValid = false;
        }
    }
    ...
    if (curSectionName != "SCHEDULE") {
        ...
        std::string msg = "The last section of a valid deck must be SCHEDULE (is " + curSectionName + ")";
        OpmLog::warning(Log::fileMessage(curKeyword.location(), msg));
        deckValid = false;
    }
